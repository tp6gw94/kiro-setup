...
基於 Kiro CLI 的多代理 AI 編碼協調器。包含 14 個專業代理、9 個自訂 Hook、24 個技能，以及程式碼生成設定管線。
...
| `phase-reminder.sh` | `userPromptSubmit` | `code_supervisor` | 每次 prompt 注入強制工作提醒，涵蓋 plan 檢查、委派、執行與驗證 |
| `cmux-notify.sh` | `stop` | `code_supervisor` | 回應完成時透過 cmux 發送桌面通知 |
| `validate-read-allowed-paths.sh` | `preToolUse` | `code_supervisor` | 阻擋讀取 `.plan/`、Kiro home、`/var/folders` 以外的路徑 |
| `validate-write-plan-folder.sh` | `preToolUse` | `.plan` artifact writers | 阻擋寫入 `.plan/` 以外的路徑 |
| `validate-developer-plan.sh` | `preToolUse` | source-writing agents | 除非 `.plan/.active-developer-plan` 指向含有 `task.md` 的任務資料夾，否則阻擋寫檔、code、shell 工具 |

### Hook 注入邏輯

`generate-configs.sh` 中定義了六個注入函式，在生成每個代理的 JSON 設定時依序呼叫：

- `inject_developer_plan_hook` — 對 source-writing agents 加入 active plan gate（匹配 `write`、`code`、`execute_bash`）
- `inject_plan_folder_write_hook` — 對 `.plan` artifact writers 加入 write path gate（匹配 `write`）
