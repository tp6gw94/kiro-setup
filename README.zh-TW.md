...
基於 Kiro CLI 的多代理 AI 編碼協調器。包含 14 個專業代理、11 個自訂 Hook、24 個技能，以及程式碼生成設定管線。

所有代理設定（`.json`）皆由 `generate-configs.sh` 在執行時產生，不納入版本控制。Prompt 檔案（`.md`）與 Hook 腳本才是 Git 追蹤的真實來源。
...
本設定使用 13 個自訂 Hook，透過不同觸發時機注入行為：

| Hook | 觸發時機 | 適用範圍 | 說明 |
|------|----------|----------|------|
| `shell/rtk-rewrite.js` | `preToolUse`（shell） | 大部分 leaf agent | 攔截 shell 指令，透過 RTK 重寫以節省 token |
| `shell/rtk-rules.sh` | `agentSpawn` | 大部分 leaf agent | 啟動時注入 RTK 使用說明（加 `rtk` 前綴） |
| `shell/validate-local-rm.js` | `preToolUse`（shell） | `developer` | 阻擋解析後落在目前工作目錄外的 `rm` 目標 |
| `caveman.sh` | `agentSpawn` | 所有代理 | 注入原始人說話風格 |
| `locale.sh` | `agentSpawn` | 所有代理 | 注入繁體中文語系指令 |
| `code_supervisor/phase-reminder.sh` | `userPromptSubmit` | `code_supervisor` | 每次 prompt 注入強制工作提醒，涵蓋 plan 檢查、委派、執行與 artifact-based 驗證 |
| `code_supervisor/cmux-notify.sh` | `stop` | `code_supervisor` | 回應完成時透過 cmux 發送桌面通知 |
| `code_supervisor/validate-read-allowed-paths.js` | `preToolUse` | `code_supervisor` | 阻擋讀取 `.plan/`、Kiro home、`/var/folders` 以外的路徑 |
| `code_supervisor/validate-supervisor-plan-write.js` | `preToolUse` | `code_supervisor` | 阻擋 supervisor 寫 planner-owned 檔案，且只允許啟用 planner-ready plan |
| `planner/validate-planner-plan-write.js` | `preToolUse` | `planner` | 允許 planner 在 `.plan/` 內寫正式 plan 檔案 |
| `planner/open-task-markdown.js` | `postToolUse` | `planner` | cmux 可用時，在首次成功寫入 `.plan/<task>/task.md` 後用 cmux markdown 開啟 |
| `plan_writers/validate-artifact-plan-write.js` | `preToolUse` | `.plan` artifact writers | 阻擋寫入 `.plan/` 以外的路徑，並保護 planner-owned 檔案 |
| `source_writing/validate-developer-plan.js` | `preToolUse` | source-writing agents | 除非 `.active-developer-plan` 指向 planner-ready 任務資料夾，否則阻擋寫檔、code、shell 工具 |

### Hook 注入邏輯

`generate-configs.sh` 中定義了九個注入函式，在生成每個代理的 JSON 設定時依序呼叫：

- `inject_developer_plan_hook` — 對 source-writing agents 加入 active plan gate（匹配 `write`、`code`、`shell`）
- `inject_developer_rm_hook` — 對 `developer` 加入本地 `rm` 安全閘（匹配 `shell`）
- `inject_plan_folder_write_hook` — 對 `.plan` artifact writers 加入 artifact write gate（匹配 `write`）
- `inject_planner_plan_write_hook` — 對 `planner` 加入正式 plan write gate（匹配 `write`）
- `inject_planner_task_markdown_hook` — 對 `planner` 加入 `task.md` markdown auto-open hook（匹配 `write` 的 `postToolUse`）
- `inject_supervisor_plan_write_hook` — 對 `code_supervisor` 加入 supervisor coordination write gate（匹配 `write`）
- `inject_rtk_hook` — 加入 `preToolUse` hook（匹配 `shell`）
...
注意：`planner`、`code_supervisor`、`researcher` 及所有 council agent 不注入 RTK hook（因為它們不直接執行 shell 指令或有其他考量）。
...
1. **`agentSpawn` hook（`shell/rtk-rules.sh`）**：代理啟動時注入提示，告知代理所有 shell 指令都應加上 `rtk` 前綴
2. **`preToolUse` hook（`shell/rtk-rewrite.js`）**：攔截實際的 shell 呼叫，透過 `rtk rewrite` 自動重寫指令
...
`code_supervisor/cmux-notify.sh` hook（在 `code_supervisor` 的 `stop` 事件觸發）會在協調器完成回應時透過 `cmux notify` 發送桌面通知。cmux 側邊欄的分頁會亮起藍色通知環，顯示專案名稱與回應預覽 — 在跨工作區管理多個 Kiro CLI 會話時非常實用。

`planner/open-task-markdown.js` hook 會在 planner 首次成功寫入 `.plan/<task>/task.md` 後用 `cmux markdown` 開啟閱讀面板，後續更新交給 live reload。這些 cmux hooks 都會先檢查可用性（`cmux ping`）；若 cmux 未安裝或未運行，會靜默跳過不執行。
...
| `.planner-ready.json` | planner | `questions.md` 為 `NO_QUESTIONS` 時寫入的 readiness marker |
...
每個代理只負責一件事。`code_supervisor` 不寫程式碼、不持有 shell、不自行跑 build/test/lint/typecheck；它只讀 `.plan` artifact 確認流程狀態，缺少驗證證據時委派 `tester` 或 `reviewer`。`developer` 不做審查，`reviewer` 不做實作。工具權限也嚴格限縮。
...
`code_supervisor` 的 6 階段工作流程（理解 → 路徑選擇 → 委派檢查 → 拆分並行 → 執行 → artifact 驗證）支援在同一輪次中平行啟動多個獨立的 subagent。
