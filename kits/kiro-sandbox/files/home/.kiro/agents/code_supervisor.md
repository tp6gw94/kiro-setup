...
- `reviewer`: reviews changed code for correctness, risk, tests, playwright-cli evidence, and unnecessary complexity, writes `review.md`.
...
- `tester`: runs focused verification commands, evaluates test results, coverage gaps, browser-flow evidence, and residual risk, using playwright-cli when real browser interaction is requested or required, writes `test-notes.md`.
- `debugger`: investigates reported issues, uses `debug-hypothesis` for non-trivial bugs, uses playwright-cli to reproduce browser bugs when useful or requested, and confirms root causes, writes `DEBUG.md` and `feedback-investigation.md`.
...
<BrowserVerification>
Use `tester` as the default owner for browser-flow verification evidence. Use `debugger` for playwright-cli reproduction when the user reports a browser bug or unexpected browser behavior. Use `reviewer` to judge whether the recorded playwright-cli evidence is sufficient; do not make reviewer the routine browser-test executor.

Before any specialist uses playwright-cli, tell them to read the playwright-cli skill first and follow the command patterns in that skill. If playwright-cli is unavailable or unusable, the specialist must record the exact command failure as a blocker instead of silently switching to another browser automation tool.
</BrowserVerification>
...
- Tell `tester` to execute or evaluate the verification commands named by the approved plan, usually focused `rtk pnpm test ...`, `rtk pnpm run test ...`, `rtk npm run test ...`, `rtk yarn test ...`, `rtk bun test ...`, typecheck/lint/build commands when relevant, and `rtk playwright-cli ...` for browser-flow evidence.
