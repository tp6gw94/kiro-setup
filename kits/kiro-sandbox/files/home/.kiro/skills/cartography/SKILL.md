...
**First, check if `.kiro/cartography.json` exists in the repo root.**
...
```bash
python3 ~/.kiro/skills/cartography/scripts/cartographer.py init \
  --root ./ \
  --include "src/**/*.ts" \
...
This creates:
- `.kiro/cartography.json` - File and folder hashes for change detection
- Empty `codemap.md` files in all relevant subdirectories

4. **Delegate to Explorer agents** - Dispatch explorer subagent stages per folder to read code and fill in its specific `codemap.md` file.
...
```bash
python3 ~/.kiro/skills/cartography/scripts/cartographer.py changes \
  --root ./
```
...
3. **Only update affected codemaps** - Dispatch explorer subagent stages per folder to update its `codemap.md`.
4. **Run update** to save new state:

```bash
python3 ~/.kiro/skills/cartography/scripts/cartographer.py update \
  --root ./
```
...
### Step 5: Register in `kiro.md` or `.kiro/steering/`

**Kiro auto-loads `kiro.md` into agent context on every session.** To ensure agents automatically discover and use the codemap, update (or create) `kiro.md` at the repo root:

1. If `kiro.md` already exists and already contains a `## Repository Map` section, **skip this step** — the reference is already set up.
2. If `kiro.md` exists but has no `## Repository Map` section, **append** the section below.
3. If `kiro.md` doesn't exist, **create** it with the section below.
